...
Author: Kulverstukas && Factionwars
...
parser.add_option('--screen',
                  help='',
                  metavar='main         Which screen to use to start D0xBase\n           add\n           search')
parser.add_option('--import',help='For importing a record from XML', metavar='import.xml')
(options, args) = parser.parse_args()
options = vars(options)

#Check if appropriate version is being used
if urwid.__version__ == '1.0.2':
    config = D0xConfigWizard.ConfigWizard().checkIfCorrupt(ConfigParser.RawConfigParser())
    #intialize database
    database =  D0xDBManager.DBManager(config.get('main', 'database_path'))
    #following command creates database file if it is missing with default record
    database.connect()
